...
def process_data_files():
  ...
  for dirpath, dirnames, files in os.walk('arff/phone/accel'):
    ...
    for file_name in files:
      ...
      for col in df_data.iloc[:,0:43]:  # Delete columns of published data
        df_data = df_data.drop(columns=[col])
      df_data = df_data.drop(columns=['a_RESULTANT'])  # Delete RESULTANT column (unused for this model)
  ...
  for dirpath, dirnames, files in os.walk('arff/phone/gyro'):
    ...
    for file_name in files:
      ...
      for col in df_data.iloc[:,0:43]:  # Delete columns of published data
        df_data = df_data.drop(columns=[col])
      df_data = df_data.drop(columns=['g_RESULTANT'])  # Delete RESULTANT column (unused for this model)
...
# process_data_files()
# pickle.dump(all_data, open("checkpoints/all_data_alt.p", 'wb'))
# pickle.dump(subjects, open("checkpoints/subjects_alt.p", 'wb'))

#print("Loading data and subjects from pickle files")
all_data = pickle.load(open("checkpoints/all_data_alt.p", "rb"))
subjects = pickle.load(open("checkpoints/subjects_alt.p", "rb"))
model_paths = pickle.load(open("checkpoints/model_paths_alt.p", "rb"))
...
for subject in subjects:
  ...
  # Save model to disk
  # model_dir = "checkpoints/alt_model_%s" % subject
...
# pickle.dump(model_paths, open("checkpoints/model_paths_alt.p", 'wb'))
